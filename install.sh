...
# ═══════════════════════════════════════════════════════════════════════════
#  Step 4: Update docker-compose.yml dengan env vars
# ═══════════════════════════════════════════════════════════════════════════

# Baca variabel dari .env
set -a; source .env 2>/dev/null || true; set +a
DB_URL="${DATABASE_URL:-mysql://root:wmspassword@wms-db:3306/wms_2026}"
DB_PASS="${MYSQL_ROOT_PASSWORD:-wmspassword}"
DB_NAME="${MYSQL_DATABASE:-wms_2026}"
SECRET="${AUTH_SECRET:-wms2026-default-secret-changeme}"
SEED_USER="${SEED_ADMIN_USERNAME:-admin}"
SEED_PASS="${SEED_ADMIN_PASSWORD:-!Tahun2026}"

# Generate docker-compose.yml dengan values dari .env
...
echo "    Backup DB : docker exec wms-db mysqldump -u root -p'${DB_PASS}' ${DB_NAME} > backup.sql"
echo "══════════════════════════════════════════════════"
echo ""
